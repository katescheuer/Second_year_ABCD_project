...
### for "cross-sectional data" ie split into subjects in yr 4 and subjects in
### baseline (but not year 4)
# select only relevant columns from combined data, rename so more understandable
data_all_cross <- data_combined %>%
...
### for all subjects, at baseline timepoint only
# select only relevant columns from combined data, rename so more understandable
data_all_base <- data_base_with_le %>% 
...
### "cross-sectional" - for measures administered at both baseline and yr 4
data_all_cross$age_group <- as.factor(data_all_cross$age_group)
nrow(data_all_cross) #11868 subjects total
## data_all_cross is split into younger and older
data_all_cross %>% 
