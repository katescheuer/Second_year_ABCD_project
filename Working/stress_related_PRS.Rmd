...
test <- as.data.frame(cbind(
                            as.numeric(predict(cbcl_total_M1a_gender)),
                            as.numeric(scale(as.numeric(predict(cbcl_total_M1a_gender)))),
                            summary(cbcl_total_M1a_gender)$residuals,
                            as.numeric(scale(summary(cbcl_total_M1a_gender)$residuals))))
colnames(test) <- c("M3_Yhat","ZM3_Yhat","M3_L1resid","ZM3_L1resid")
# yr4_reg_data$ZM3_L1resid = as.numeric(scale(yr4_reg_data$M3_L1resid))
...
### general: temporaryfile <- ranef(modelname)$L2IDvariable["nameofcoefficient"]
...
# slope_iqverbal <- ranef(cbcl_total_M1a_gender)$site["ZIQ_verb_CMC"]
# colnames(slope_iqverbal) = c("RE_iqverbal")
# rownames(slope_iqverbal) <- NULL
# slope_minority <- ranef(cbcl_total_M1a_gender)$site["minority_CMC"]
# colnames(slope_minority) = c("RE_minority")
# rownames(slope_minority) <- NULL
### standardize (z-score) the residuals
intercept$ZRE_int <- as.numeric(scale(intercept$RE_int), scale = TRUE)
# slope_iqverbal$ZRE_iqverbal <- as.numeric(scale(slope_iqverbal$RE_iqverbal), scale = TRUE)
# slope_minority$ZRE_minority <- as.numeric(scale(slope_minority$RE_minority), scale = TRUE)
### combine the L2 residuals
# RE_file <- bind_cols(intercept,slope_iqverbal,slope_minority)
RE_file <- intercept
### add L2id to the L2 residuals (the L2id will be in same descending order; make sure L2agg ID is in descending order)
RE_file$site <- row.names(ranef(cbcl_total_M1a_gender)$site)
# RE_file$site <- yr4_reg_data$site
### merge to L2 aggregate file by L2id
# L2agg <- merge(L2agg,RE_file,by = "site")
...
plot(test$ZM3_Yhat, 
     test$ZM3_L1resid, 
     main = "L1 Predicted vs. Residual Plot",
     xlab = "Predicted Values (Z)", 
     ylab = "Residual Values (Z)",
     pch=1, col = "black")
abline(lm(test$ZM3_L1resid ~ test$ZM3_Yhat), col="red")
...
### check L1 residuals
hist(test$ZM3_L1resid)
# hist(yr4_reg_data$ZM3_L1resid) #we see some left skew but fairly bell shaped

### check L2 residuals **from 2L** file
hist(RE_file$ZRE_int)
# hist(L2agg$ZRE_int) #we see fairly symmetric bell curve (appears normal)
# hist(L2agg$ZRE_iqverbal) #we see fairly symmetric bell curve (appears normal)
# hist(L2agg$ZRE_minority) #we see fairlysymmetric bell curve (appears normal)
...
#### level 1 residuals ----
skewness(test$ZM3_L1resid)
kurtosis(test$ZM3_L1resid)
#### level 2 residuals ----
##### intercept ----
skewness(RE_file$ZRE_int) 
kurtosis(RE_file$ZRE_int) 
##### random slope for verbal relation with DV ----
# skewness(L2agg$ZRE_iqverbal) 
# kurtosis(L2agg$ZRE_iqverbal) 
##### random slope for minoritized student relation with DV ----
# skewness(L2agg$ZRE_minority) 
# kurtosis(L2agg$ZRE_minority) 
...
#### level 1 residuals
##### there is significant (left) skew and kurtosis (leptokurtosis/pointy) in L1 residuals
agostino.test(test$ZM3_L1resid, alternative = c("two.sided"))
anscombe.test(test$ZM3_L1resid, alternative = c("two.sided"))
#### level 2 residuals
##### there is no significant skew or kurtosis in L2 residuals
##### intercept
agostino.test(RE_file$ZRE_int, alternative = c("two.sided"))
anscombe.test(RE_file$ZRE_int, alternative = c("two.sided"))
##### verbal slope
# agostino.test(L2agg$ZRE_iqverbal, alternative = c("two.sided"))
# anscombe.test(L2agg$ZRE_iqverbal, alternative = c("two.sided"))
##### minoritized student status slope
# agostino.test(L2agg$ZRE_minority, alternative = c("two.sided"))
# anscombe.test(L2agg$ZRE_minority, alternative = c("two.sided"))
