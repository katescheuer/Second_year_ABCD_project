...
### Compare gender vs sex models for eur ancestry only (too few gnc in afr or amadmix)
...
### (if fan shape)
### Robust SEs ----
### Let’s move forward now to estimate our model using “robust” SEs that are fitted using an 
...
``` {eur ksads~prs and covar regressions - adhd,r}
...
### Random Intercept, Fixed Slopes without interactions ----
ksads_adhd_M1a_gender <- lmer(res_k_adhd ~ zcmc_prs + zcmc_age + zcmc_income +c
                              gen_cmc_cisf + gen_cmc_gnc +
                            (1|site),
                          data=yr4_reg_data, REML=FALSE)
...
anova(ksads_adhd_M1a_sex,ksads_adhd_M2a_sex,test="Chisq") #1a is better
anova(ksads_adhd_M1a_sex,ksads_adhd_M2b_sex,test="Chisq") #1a is better
anova(ksads_adhd_M1a_sex,ksads_adhd_M2c_sex,test="Chisq") #1a is better
anova(ksads_adhd_M1a_sex,ksads_adhd_M2d_sex,test="Chisq") #1a is better

anova(ksads_adhd_M1a_gender,ksads_adhd_M1b_gender,test="Chisq")  #1a is better
anova(ksads_adhd_M1a_gender,ksads_adhd_M2a_gender,test="Chisq") #1a is better
anova(ksads_adhd_M1a_gender,ksads_adhd_M2b_gender,test="Chisq") #1a is better
anova(ksads_adhd_M1a_gender,ksads_adhd_M2c_gender,test="Chisq") #1a is better
anova(ksads_adhd_M1a_gender,ksads_adhd_M2d_gender,test="Chisq") #1a is better

# 1a gender: bic = 16517.1        
# 1a sex: bic = 16509.3          
# all: choose model 1a sex
eur_ksads_adhd_M1a_gender <- ksads_adhd_M1a_gender
eur_ksads_adhd_M1a_sex <- ksads_adhd_M1a_sex

### Sig Results ----
## cisf: p = 0.03517  , beta = -0.43735                        

### ext variance explained by model (r^2) ----
#### 1 - ((residual variance)/(original ext variance)) # 1 - % unexplained variance = % explained
#### orig_Y_var = var(dataframe$outcome) to get original Y variance
orig_Y_var = var(yr4_reg_data$res_k_adhd) 
orig_Y_var
#### model_variances = as.data.frame(VarCorr(modelnamehere)) to obtain variance estimates
model_variances = as.data.frame(VarCorr(ksads_adhd_M1a_sex)) 
model_variances
#### res_var = model_variances[rownumber,'vcov'] to get L1 residual variance; look at model_variances first
res_var = model_variances[2,'vcov'] 
res_var
#### calculate
R2_adhd <- 1 - res_var/orig_Y_var
R2_adhd
...
### % of variance in Y "uniquely" explained by that predictor (not overlapping with other Xs)
### extract residual df 
res_df <- as.numeric(summary(ksads_adhd_M1a_sex)$AICtab[5])
res_df 
### extract t-values
tvals <- data.frame(cbind(row.names(summary(ksads_adhd_M1a_sex)$coef), #predictor names
                          summary(ksads_adhd_M1a_sex)$coef[,4]) #t values
              )
colnames(tvals) <- c("predictor","tval")
tvals$tval <- as.numeric(tvals$tval)
### now compute the approx sr^2 values
...
# Extract L1 Predicted and Residual Values to Add to Main L1 Dataset ----
### predicted values (Yhats)
yr4_reg_data$ksads_adhd_M1a_sex_Yhat <- as.numeric(predict(ksads_adhd_M1a_sex))
# create Z score version
yr4_reg_data$Zksads_adhd_M1a_sex_Yhat <- as.numeric(scale(yr4_reg_data$ksads_adhd_M1a_sex_Yhat))
### residual values (the r's, Y - Yhat)
yr4_reg_data$ksads_adhd_M1a_sex_L1resid <- as.numeric(resid(ksads_adhd_M1a_sex))
# create Z score version
yr4_reg_data$Zksads_adhd_M1a_sex_L1resid <- as.numeric(
                                                scale(yr4_reg_data$ksads_adhd_M1a_sex_L1resid))

## Extract L2 Predicted and Residual Values to Add to Previously Built L2 Aggregate Dataset ----
### no L2 predictors so only normality and independence assessed
### extract L2 residuals using ranef() function
intercept <- ranef(ksads_adhd_M1a_sex)$site["(Intercept)"] #intercept is always as "(Intercept)"
colnames(intercept) = c("RE_int")
rownames(intercept) <- NULL
intercept$ZRE_int <- as.numeric(scale(intercept$RE_int), scale = TRUE)
RE_file <- intercept
RE_file$site <- row.names(ranef(ksads_adhd_M1a_sex)$site)

## Assumption #1 and 2 Linearity and Heteroskedasticity ----
### visually inspect residual-predicted plot for evidence of curvilinearity
### level 1 ----

ggplot(aes(x=Zksads_adhd_M1a_sex_Yhat,
           y=Zksads_adhd_M1a_sex_L1resid),data=yr4_reg_data) +
  geom_point(aes(color=sex)) +
  labs(x="Predicted Values (Z)",y="Residual Values (Z)") +
  geom_smooth(method="lm",color="black") +
  theme_bw()
...
### general: use rlmer() function from “robustlmm” package, instead of lmer() in lme4 package
### note that this function takes a few minutes to estimate, keep your patience hat on /!\ 
ksads_adhd_M1a_sex_robustSE <- rlmer(res_k_adhd ~ zcmc_prs + zcmc_age + zcmc_income +
                              sex_cmc_cisf +
                            (1|site),
                          data=yr4_reg_data, REML=FALSE)
summary(ksads_adhd_M1a_sex_robustSE)
summary(ksads_adhd_M1a_sex) # for comparison with the robust estimate
### results substantively the same (what was significant remains sig, what was not is not sig)
### so, non-normality present in our data does not alter our conclusions
eur_ksads_adhd_M1a_sex_robustSE <- ksads_adhd_M1a_sex_robustSE
# similar enough that fan pattern not concerning

data.frame(summary(ksads_adhd_M1a_sex_robustSE)$coefficients) %>%
  rename(robust_est = Estimate,
         robust_se = Std..Error,
         robust_tval = t.value) %>%
  bind_cols(summary(ksads_adhd_M1a_sex)$coef) %>%
  rename(notrobust_est = Estimate,
         notrobust_se = `Std. Error`,
         notrobust_df = `df`,
         notrobust_tval = `t value`,
...
### visually inspect residuals (skew/kurtosis tests can miss multimodality!) ----
### check L1 residuals
hist(yr4_reg_data$Zksads_adhd_M1a_sex_L1resid)
...
### formal statistical tests of skew and kurtosis
#### level 1 residuals
agostino.test(yr4_reg_data$Zksads_adhd_M1a_sex_L1resid, alternative = c("two.sided"))
anscombe.test(yr4_reg_data$Zksads_adhd_M1a_sex_L1resid, alternative = c("two.sided"))
#### level 2 residuals
agostino.test(RE_file$ZRE_int, alternative = c("two.sided"))
anscombe.test(RE_file$ZRE_int, alternative = c("two.sided"))
...
### Run regression for afr and amadmix CBCL outcomes ~ PRS + income-to-needs + age + sex at yr4
