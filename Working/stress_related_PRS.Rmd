...
## join dataframes with abcd info and residuals ####
## all
yr4_reg_all_data <- yr4_reg_data %>% 
...
## american admixed ancestry
yr4_reg_amadmix_data <- yr4_reg_data %>% 
        filter(ancestry == "AmAdmix") %>%
        left_join(lm_all_resid_cbcl,by="id") %>%
        left_join(lm_all_resid_ksads,by="id") %>%
        rename(
                resid_cbcl_aggress = cbcl_scr_syn_aggressive_t,
...
cbcl_total_M0 <- lmer(resid_cbcl_total ~ 1 + (1|site), data=yr4_reg_all_data, REML=FALSE)
...
gender_M0 <- lmer(gender ~ (1|site), data=yr4_reg_all_data, REML=FALSE)
summary(gender_M0)
...
income_to_needs_M0 <- lmer(income_to_needs ~ (1|site), data=yr4_reg_all_data, REML=FALSE)
...
age_M0 <- lmer(age ~ (1|site), data=yr4_reg_all_data, REML=FALSE)
summary(age_M0)
...
prs_M0 <- lmer(prs ~ (1|site), data=yr4_reg_all_data, REML=FALSE)
summary(prs_M0)
...
yr4_reg_all_data$female_eff = as.numeric(car::recode(yr4_reg_all_data$gender, "1=1;0=-1")) 
