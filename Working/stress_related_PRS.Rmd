...
ksads_lifetime <-
  read.csv("ABCD_Baseline_T1_T2_T3_T4_KSADS_Lifetime_Diagnoses(in).csv") #lifetime ksads diagnoses
le_p <-
  read.csv("mh_p_le.csv", header = T) #parent-reported life events scale
le_y <-
  read.csv("mh_y_le.csv", header = T) #parent-reported life events scale
...
## PRS
prs_stress <-
  read.table("Mendel_Rand_StressPRS.profile", header = T) #includes only snps linked to psych disorder via mendelian randomization (table S6 in Penner-Goeke 2023)
prs_eur_mdd <-
  read.table("renamed_scz_ABCD_imputed-MDD_EUR_HOWARD2019-EUR.profiles",
             header = T)
prs_afr_mdd <-
  read.table("renamed_scz_ABCD_imputed-MDD_AFR_MENG2024-AFR.profiles",
             header = T)
prs_amadmix_mdd <-
  read.table("renamed_scz_ABCD_imputed-MDD_HIS_MENG2024-AMR.profiles",
             header = T)
...
## load grm with all participants
all_grm <-
  readRDS(file = "ABCD_All_PCRel_grm_sparse.rda") # all ancestries

#if remove first 10 char from names in grm and prs files so just left with subject id
rownames(all_grm) <- gsub("^.{1,10}", "", rownames(all_grm))
colnames(all_grm) <- gsub("^.{1,10}", "", colnames(all_grm))
prs_eur_mdd <-
  prs_eur_mdd %>% mutate(src_subject_id = gsub("^.{1,10}", "", prs_eur_mdd$IID))
prs_afr_mdd <-
  prs_afr_mdd %>% mutate(src_subject_id = gsub("^.{1,10}", "", prs_afr_mdd$IID))
prs_amadmix_mdd <-
  prs_amadmix_mdd %>% mutate(src_subject_id = gsub("^.{1,10}", "", prs_amadmix_mdd$IID))

#split grm with all participants into grm based on ancestry
eur_grm <-
  all_grm[rownames(all_grm) %in% prs_eur_mdd$src_subject_id,
          colnames(all_grm) %in% prs_eur_mdd$src_subject_id]
afr_grm <-
  all_grm[rownames(all_grm) %in% prs_afr_mdd$src_subject_id,
          colnames(all_grm) %in% prs_afr_mdd$src_subject_id]
amadmix_grm <-
  all_grm[rownames(all_grm) %in% prs_amadmix_mdd$src_subject_id,
          colnames(all_grm) %in% prs_amadmix_mdd$src_subject_id]
...
## create combined data frame for parent and youth life events reports
le_combo <- le_p %>%
            # combine all columns from parent and youth report into one data frame
            left_join(le_y,by=c("src_subject_id","eventname")) %>%
            # create new row for each item 
            # with 1 if either parent or youth says yes else 0 if either parent or youth says no
            mutate(died_c = case_when(ple_died_p==1|ple_died_y==1 ~ 1, #if either say yes then 1
                                      ple_died_p==0|ple_died_y==0 ~ 0),  #else if either say no then 0, else NA
                   injured_c = case_when(ple_injured_p==1|ple_injured_y==1 ~ 1, 
                                         ple_injured_p==0|ple_injured_y==0 ~ 0),
                   crime_c = case_when(ple_crime_p==1|ple_crime_y==1 ~ 1,
                                         ple_crime_p==0|ple_crime_y==0 ~ 0),
                   friend_c = case_when(ple_friend_p==1|ple_friend_y==1 ~ 1,
                                         ple_friend_p==0|ple_friend_y==0 ~ 0),
                   friend_injur_c = case_when(ple_friend_injur_p==1|ple_friend_injur_y==1 ~ 1,
                                         ple_friend_injur_p==0|ple_friend_injur_y==0 ~ 0),
                   financial_c = case_when(ple_financial_p==1|ple_financial_y==1 ~ 1,
                                         ple_financial_p==0|ple_financial_y==0 ~ 0),
                   sud_c = case_when(ple_sud_p==1|ple_sud_y==1 ~ 1,
                                         ple_sud_p==0|ple_sud_y==0 ~ 0),
                   ill_c = case_when(ple_ill_p==1|ple_ill_y==1 ~ 1,
                                         ple_ill_p==0|ple_ill_y==0 ~ 0),
                   injur_c = case_when(ple_injur_p==1|ple_injur_y==1 ~ 1,
                                         ple_injur_p==0|ple_injur_y==0 ~ 0),
                   argue_c = case_when(ple_argue_p==1|ple_argue_y==1 ~ 1,
                                         ple_argue_p==0|ple_argue_y==0 ~ 0),
                   job_c = case_when(ple_job_p==1|ple_job_y==1 ~ 1,
                                         ple_job_p==0|ple_job_y==0 ~ 0),
                   away_c = case_when(ple_away_p==1|ple_away_y==1 ~ 1,
                                         ple_away_p==0|ple_away_y==0 ~ 0),
                   arrest_c = case_when(ple_arrest_p==1|ple_arrest_y==1 ~ 1,
                                         ple_arrest_p==0|ple_arrest_y==0 ~ 0),
                   friend_died_c = case_when(ple_friend_died_p==1|ple_friend_died_y==1 ~ 1,
                                         ple_friend_died_p==0|ple_friend_died_y==0 ~ 0),
                   mh_c = case_when(ple_mh_p==1|ple_mh_y==1 ~ 1,
                                         ple_mh_p==0|ple_mh_y==0 ~ 0),
                   sib_c = case_when(ple_sib_p==1|ple_sib_y==1 ~ 1,
                                         ple_sib_p==0|ple_sib_y==0 ~ 0),
                   victim_c = case_when(ple_victim_p==1|ple_victim_y==1 ~ 1,
                                         ple_victim_p==0|ple_victim_y==0 ~ 0),
                   separ_c = case_when(ple_separ_p==1|ple_separ_y==1 ~ 1,
                                         ple_separ_p==0|ple_separ_y==0 ~ 0),
                   law_c = case_when(ple_law_p==1|ple_law_y==1 ~ 1,
                                         ple_law_p==0|ple_law_y==0 ~ 0),
                   school_c = case_when(ple_school_p==1|ple_school_y==1 ~ 1,
                                         ple_school_p==0|ple_school_y==0 ~ 0),
                   move_c = case_when(ple_move_p==1|ple_move_y==1 ~ 1,
                                         ple_move_p==0|ple_move_y==0 ~ 0),
                   jail_c = case_when(ple_jail_p==1|ple_jail_y==1 ~ 1,
                                         ple_jail_p==0|ple_jail_y==0 ~ 0),
                   step_c = case_when(ple_step_p==1|ple_step_y==1 ~ 1,
                                         ple_step_p==0|ple_step_y==0 ~ 0),
                   new_job_c = case_when(ple_new_job_p==1|ple_new_job_y==1 ~ 1,
                                         ple_new_job_p==0|ple_new_job_y==0 ~ 0),
                   new_sib_c = case_when(ple_new_sib_p==1|ple_new_sib_y==1 ~ 1,
                                         ple_new_sib_p==0|ple_new_sib_y==0 ~ 0),
                   hospitalized_c = case_when(ple_hospitalized_p==1|ple_hospitalized_y==1 ~ 1,
                                         ple_hospitalized_p==0|ple_hospitalized_y==0 ~ 0),
                   foster_care_c = case_when(ple_foster_care_p==1|ple_foster_care_y==1 ~ 1,
                                         ple_foster_care_p==0|ple_foster_care_y==0 ~ 0),
                   hit_c = case_when(ple_hit_p==1|ple_hit_y==1 ~ 1,
                                         ple_hit_p==0|ple_hit_y==0 ~ 0),
                   shot_c = case_when(ple_shot_p==1|ple_shot_y==1 ~ 1,
                                         ple_shot_p==0|ple_shot_y==0 ~ 0),
                   lockdown_c = case_when(ple_lockdown_p==1|ple_lockdown_y==1 ~ 1,
                                         ple_lockdown_p==0|ple_lockdown_y==0 ~ 0),
                   homeless_c = case_when(ple_homeless_p==1|ple_homeless_y==1 ~ 1,
                                         ple_homeless_p==0|ple_homeless_y==0 ~ 0),
                   deported_c = case_when(ple_deported_p==1|ple_deported_y==1 ~ 1,
                                         ple_deported_p==0|ple_deported_y==0 ~ 0),
                   suicide_c = ple_suicide_y #not asked of parents
                   ) %>%
              #select only columns with combined information
              select(src_subject_id,eventname,
                     died_c,injured_c,crime_c,friend_c,friend_injur_c,financial_c,
                     sud_c,ill_c,injur_c,argue_c,job_c,away_c,arrest_c,friend_died_c,
                     mh_c,sib_c,victim_c,separ_c,law_c,school_c,move_c,jail_c,
                     step_c,new_job_c,new_sib_c,hospitalized_c,foster_care_c,
                     hit_c,shot_c,lockdown_c,homeless_c,deported_c,suicide_c) %>%
              #sum number of events experienced
              rowwise() %>%
              mutate(le_total = sum(
                     died_c,injured_c,crime_c,friend_c,friend_injur_c,financial_c,
                     sud_c,ill_c,injur_c,argue_c,job_c,away_c,arrest_c,friend_died_c,
...
abcd_all <- demo %>%
              ...
              left_join(select(ksads_lifetime,
                               c("src_subject_id","eventname",
                                 # note that using release 3 criteria for ADHD, see slack from Sam
                                 "lifetime_adhd_release3_criteria",
                                 "umbrella_anxiety_disorder",
                                 "lifetime_mdd","lifetime_ptsd")),
                        by=c("src_subject_id","eventname")) %>%
              rename(adhd = lifetime_adhd_release3_criteria,
                     anxiety = umbrella_anxiety_disorder,
                     mdd = lifetime_mdd,
                     ptsd = lifetime_ptsd) %>%
              # add combined parent and youth report life events info
              left_join(select(le_combo,
                               c("src_subject_id","eventname","le_total")),
                        by=c("src_subject_id","eventname"))
...
prs_all <- prs_stress %>%
            ...
            select(c(src_subject_id,prs_stress))
...
data_all <- abcd_all %>%
              ...
              select(c(src_subject_id,eventname,site,ancestry,
                       sex,gender,income_to_needs,le_total,age,
                       Zprs_stress,
                       # Zprs_mdd,
...
lm_eur_stressprs_total <- fitNullModel(eur_yr4_data_reordered, 
                                  outcome = "cbcl_total", 
                                  covars = c("Zprs_stress_cmc", 
                                             # "income_to_needs",
                                             # "le_total",
                                             "sex_cmc_cisf", "age_zcmc",
                                             # "Zprs_stress_cmc*income_to_needs",
                                             # "Zprs_stress_cmc*le_total",
                                             # "Zprs_stress_cmc*sex_cmc_cisf",
                                             # "Zprs_stress_cmc*age_zcmc",
                                             paste0("V", 1:8)),
...
lm_eur_stressprs_anxiety <- fitNullModel(eur_yr4_data_reordered, 
                                  ...
                                  covars = c("Zprs_stress_cmc", 
                                             # "le_total",
                                             "sex_cmc_cisf", "age_zcmc", paste0("V", 1:8)),
                                  # covars = c("Zprs_stress_cmc", "gender_eff_cisf", "gender_eff_gd","age_zcmc", paste0("V", 1:8)),
                                  cov.mat = list(kinship = eur_grm_sparse_subsetted,
                                                 site = eur_site_covmat),
                                  family = "binomial",
...
``` {stress PRS, eur ancestry: cbcl or ksads ~ stress prs + income_to_needs + life_events + sex + age + ancestry PCs, r}

lm_eur_stressprs_total <- fitNullModel(eur_yr4_data_reordered, 
                                  outcome = "cbcl_total", 
                                  covars = c("Zprs_stress_cmc", 
                                             # "income_to_needs", "le_total",
                                             "sex_cmc_cisf", "age_zcmc", 
                                             paste0("V", 1:8)),
                                  # covars = c("Zprs_stress_cmc", "gender_eff_cisf", "gender_eff_gd","age_zcmc", paste0("V", 1:8)),
                                  cov.mat = list(kinship = eur_grm_sparse_subsetted,
