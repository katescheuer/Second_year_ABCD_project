...
# 1a gender: aic = 14824.9, bic = 14885.6
# 1a sex: aic = 14865.1, bic = 14919.7
# all: choose model 1a gender

### Sig Results ----
## income: p = 0.0187, beta = -0.102015
## cisf: p < 0.0001, beta = -0.600201 (compared to avg)
## gnc: p < 0.0001, beta = 1.279358 (compared to avg)



### using alternative approach #1 ----
#### sometimes r2mlm doesn't work for our model; 
#### this approach is for linear models with little or no missingness and cluster-mean-centering used
#### 1 - ((residual variance)/(original total variance)) # 1 - % unexplained variance = % explained
#### orig_Y_var = var(dataframe$outcome) to get original Y variance; we have no missingness and outcome is interval or ratio scale
orig_Y_var = var(yr4_reg_data$res_c_total) 
orig_Y_var
#### model_variances = as.data.frame(VarCorr(modelnamehere)) to obtain variance estimates
model_variances = as.data.frame(VarCorr(cbcl_total_M1a_gender)) 
model_variances
...
res_var
#### calculate
R2_total_alt1 <- 1 - res_var/orig_Y_var
R2_total_alt1
R2_total <- R2_total_alt1
# [1] 0.5020215
# 50.20 % of the variance in language posttest was explained by the model


## Fixed Effect Slope Coefficient Effect Size using sr^2 ----
### sr^2 = squared semi-partial correlation
### % of variance in Y "uniquely" explained by that predictor (not overlapping with other Xs)
### sr^2 = ((pr^2)*(1-R^2))/(1-pr^2), where R^2 = total R^2 computed earlier, and pr^2 = partial correlation squared = (t^2)/((t^2)+df)
### using substitution then:
### sr2 = (( t^2 /( t^2 + df))*(1 – R^2))/(1 – ( t^2 /( t^2 + df)))
### basically, we need two things: the residual df, and the t-value for each coefficient
###
### (a) extract residual df ----
###      AIC       BIC    logLik  deviance  df.resid 
### 24876.09  24988.26 -12420.05  24840.09   3740.00 
### to extract residual df (fifth element of the lme4 "AIC" table above) we use this general code:
### res_df <- as.numeric(summary(modelname)$AICtab[5])
res_df <- as.numeric(summary(cbcl_total_M1a_gender)$AICtab[5])
res_df 
### (b) extract t-values ----
###                                Estimate Std. Error        df t value             Pr(>|t|)    
### g00 (Intercept)                40.96125    0.23886 207.41991 171.488 < 0.0000000000000002 ***
### g10 ZIQ_verb_CMC                4.61216    0.12022 182.73802  38.363 < 0.0000000000000002 ***
### g20 minority_CMC               -0.63078    0.45734  88.23155  -1.379              0.17131    
### g01 ZIQ_verb_agg                3.50781    0.27058 222.65833  12.964 < 0.0000000000000002 ***
### g02 Zminpct_agg                 0.11474    0.29303 213.66935   0.392              0.69576    
### g30 ZIQ_verb_CMC:minority_CMC   0.23775    0.27486 493.51795   0.865              0.38747    
### g11 ZIQ_verb_CMC:ZIQ_verb_agg  -0.33252    0.13612 189.70341  -2.443              0.01549 *  
### g12 ZIQ_verb_CMC:Zminpct_agg   -0.32139    0.12184  94.24653  -2.638              0.00976 ** 
### g21 minority_CMC:ZIQ_verb_agg  -0.75931    0.33680  67.52444  -2.254              0.02742 *  
### g22 minority_CMC:Zminpct_agg   -0.03575    0.29567  50.51102  -0.121              0.90424    
### g03 ZIQ_verb_agg:Zminpct_agg    0.11552    0.14997 253.95101   0.770              0.44184 
###
### above is called the "coef" table in lme4; to extract a value we can use this general code:
### g_tval <- as.numeric(summary(modelname)$coef[row,column])
zcmc_prs_tval <- summary(cbcl_total_M1a_gender)$coef[2,4]
zcmc_age_tval <- summary(cbcl_total_M1a_gender)$coef[3,4]
zcmc_income_tval <- summary(cbcl_total_M1a_gender)$coef[4,4]
cisf_tval <- summary(cbcl_total_M1a_gender)$coef[5,4]
gnc_tval <- summary(cbcl_total_M1a_gender)$coef[5,4]

# g10_tval <- summary(Posttest_M3)$coef[2,4] 
# g20_tval <- summary(Posttest_M3)$coef[3,4] 
# g01_tval <- summary(Posttest_M3)$coef[4,4] 
# g02_tval <- summary(Posttest_M3)$coef[5,4] 
# g30_tval <- summary(Posttest_M3)$coef[6,4] 
# g11_tval <- summary(Posttest_M3)$coef[7,4] 
# g12_tval <- summary(Posttest_M3)$coef[8,4] 
# g21_tval <- summary(Posttest_M3)$coef[9,4] 
# g22_tval <- summary(Posttest_M3)$coef[10,4] 
# g03_tval <- summary(Posttest_M3)$coef[11,4] 
### check to make sure they are correctly extracted
tvals <- as.data.frame(c(
zcmc_prs_tval,
zcmc_age_tval,
zcmc_income_tval,
cisf_tval,
...
tvals
### (c) now compute the approx sr^2 values ----
### note that you can use ^2 or **2 to square a value in R
zcmc_prs_sr2 <- ((zcmc_prs_tval**2/(zcmc_prs_tval**2+res_df))*(1-R2_total))/(1- (zcmc_prs_tval**2/(zcmc_prs_tval**2+res_df)))
zcmc_age_sr2 <- ((zcmc_age_tval**2/(zcmc_age_tval**2+res_df))*(1-R2_total))/(1- (zcmc_age_tval**2/(zcmc_age_tval**2+res_df)))
zcmc_income_sr2 <- ((zcmc_income_tval**2/(zcmc_income_tval**2+res_df))*(1-R2_total))/(1- (zcmc_income_tval**2/(zcmc_income_tval**2+res_df)))
cisf_sr2 <- ((cisf_tval**2/(cisf_tval**2+res_df))*(1-R2_total))/(1- (cisf_tval**2/(cisf_tval**2+res_df)))
gnc_sr2 <- ((gnc_tval**2/(gnc_tval**2+res_df))*(1-R2_total))/(1- (gnc_tval**2/(gnc_tval**2+res_df)))
# g10_sr2 <- ((g10_tval**2/(g10_tval**2+res_df))*(1-R2_total))/(1- (g10_tval**2/(g10_tval**2+res_df)))
# g20_sr2 <- ((g20_tval**2/(g20_tval**2+res_df))*(1-R2_total))/(1- (g20_tval**2/(g20_tval**2+res_df)))
# g01_sr2 <- ((g01_tval**2/(g01_tval**2+res_df))*(1-R2_total))/(1- (g01_tval**2/(g01_tval**2+res_df)))
# g02_sr2 <- ((g02_tval**2/(g02_tval**2+res_df))*(1-R2_total))/(1- (g02_tval**2/(g02_tval**2+res_df)))
# g30_sr2 <- ((g30_tval**2/(g30_tval**2+res_df))*(1-R2_total))/(1- (g30_tval**2/(g30_tval**2+res_df)))
# g11_sr2 <- ((g11_tval**2/(g11_tval**2+res_df))*(1-R2_total))/(1- (g11_tval**2/(g11_tval**2+res_df)))
# g12_sr2 <- ((g12_tval**2/(g12_tval**2+res_df))*(1-R2_total))/(1- (g12_tval**2/(g12_tval**2+res_df)))
# g21_sr2 <- ((g21_tval**2/(g21_tval**2+res_df))*(1-R2_total))/(1- (g21_tval**2/(g21_tval**2+res_df)))
# g22_sr2 <- ((g22_tval**2/(g22_tval**2+res_df))*(1-R2_total))/(1- (g22_tval**2/(g22_tval**2+res_df)))
# g03_sr2 <- ((g03_tval**2/(g03_tval**2+res_df))*(1-R2_total))/(1- (g03_tval**2/(g03_tval**2+res_df)))
### see results
sr2vals <- as.data.frame(c(
  zcmc_prs_sr2,
  zcmc_age_sr2,
  zcmc_income_sr2,
  cisf_sr2,
...
### 10 0.000078364863
### 
### The first predictor, student-level verbal skills, uniquely explained 19.44% of the variance 
### in language posttests. In contrast, the second predictor, student minority status, uniquely 
### explained less than .03% of the variance. (etc.)
### 
### to round the values to 2-decimals (APA format) use:
sr2vals_round <- round(sr2vals,2)
sr2vals_round
...
```{checking model assumptions, r}

cbcl_total_M1a_gender

# Extract L1 Predicted and Residual Values to Add to Main L1 Dataset ----
### predicted values (Yhats)
yr4_reg_data$M3_Yhat <- as.numeric(predict(cbcl_total_M1a_gender))
# create Z score version
yr4_reg_data$ZM3_Yhat <- as.numeric(scale(yr4_reg_data$M3_Yhat))
### residual values (the r's, Y - Yhat)
yr4_reg_data$M3_L1resid <- as.numeric(resid(cbcl_total_M1a_gender))
# create Z score version
yr4_reg_data$ZM3_L1resid <- as.numeric(scale(yr4_reg_data$M3_L1resid))

## Extract L2 Predicted and Residual Values to Add to Previously Built L2 Aggregate Dataset ----
### because L2 does not have predicted values that are disentangled from L1, we focus on 
### obtaining the L2 residuals and matching those with the L2 predictors 
### (if there are no L2 predictors, then linearity and homoscedasticity at L2 are irrelevant; 
### if this is your situation, only normality and independence would be assessed)
### extract L2 residuals using ranef() function
intercept <- ranef(cbcl_total_M1a_gender)$site["(Intercept)"] #intercept is always as "(Intercept)"
colnames(intercept) = c("RE_int")
rownames(intercept) <- NULL
intercept$ZRE_int <- as.numeric(scale(intercept$RE_int), scale = TRUE)
RE_file <- intercept
RE_file$site <- row.names(ranef(cbcl_total_M1a_gender)$site)

## Assumption #1 Linearity ----
### visually inspect residual-predicted plot for evidence of curvilinearity
### general: nameplot <- plot(data$predictedvariablename, data$residualvariablename)
### level 1 ----

ggplot(aes(x=ZM3_Yhat,y=ZM3_L1resid),data=yr4_reg_data) +
...
### check L1 residuals
hist(yr4_reg_data$ZM3_L1resid)

### check L2 residuals **from 2L** file
hist(RE_file$ZRE_int)

### statistically evaluate residuals for normality ----
### we will evaluate skew and kurtosis with “moments” package skewness() and kurtosis() functions
#### notes: skewness is the standardized third moment of a standardized variable (z-scores) 
#### notes: kurtosis is the standardized fourth moment of a standardized variable (z-scores) 
#### for the unit normal curve (Z), population kurtosis is the expected value of fourth power of 
#### the z-scores E(Z^4), which equals 3
#### the “psych” package defines kurtosis as E(Z^4)−3 = 0 and is thus really "excess" kurtosis
#### the “moments” package leaves kurtosis as it is: E(Z^4) = 3
#### to match with psych package kurtosis estimate, subtract 3
#### level 1 residuals ----
skewness(yr4_reg_data$ZM3_L1resid)
kurtosis(yr4_reg_data$ZM3_L1resid)
#### level 2 residuals ----
##### intercept ----
skewness(RE_file$ZRE_int) 
kurtosis(RE_file$ZRE_int) 

### formal statistical tests of skew and kurtosis
### skew test general: agostino.test(data$variablename, alternative = c("two.sided"))
### kurtosis test general: anscombe.test(data$variablename, alternative = c("two.sided"))
#### level 1 residuals
library(moments)
agostino.test(yr4_reg_data$ZM3_L1resid, alternative = c("two.sided"))
anscombe.test(yr4_reg_data$ZM3_L1resid, alternative = c("two.sided"))
#### level 2 residuals
##### intercept
...
# 1a gender: aic = 14479.6, bic = 14540.3
# 1a sex: aic = 14498.3, bic = 14552.9
# all: choose model 1a gender

### Sig Results ----
## income: p = 0.000577, beta = -0.141623
## cisf: p < 0.0001, beta = -0.490761 (compared to avg)
## gnc: p < 0.0001, beta = 0.767487 (compared to avg)
...
# 1a gender: aic = 16546.7, bic = 16607.4
# 1a sex: aic = 16582.9, bic = 16637.5
# all: choose model 1a gender

### Sig Results ----
## cisf: p < 0.0001, beta = -0.669480 (compared to avg)
## gnc: p < 0.0001, beta = 1.704216 (compared to avg)
...
# 1a gender: aic = 10927.5, bic = 10988.2
# 1a sex: aic = 10936.5, bic = 10991.1  
# all: choose model 1a gender

### Sig Results ----
## income: p = 0.000249, beta = -0.0864993
## cisf: p < 0.0001, beta = -0.2265196 (compared to avg)
## gnc: p = 0.003578, beta = 0.3007607 (compared to avg)
...
# 1a sex: aic = 14509.2, bic = 14563.9
# all: choose model 1a gender

### Sig Results ----
## cisf: p < 0.0001, beta = -0.707134 (compared to avg)
## gnc: p = 0.003567, beta = 1.440444 (compared to avg)
...
# 1a gender: aic = 14547.5, bic = 14608.1
# 1a sex: aic = 14577.3, bic = 14631.9
# all: choose model 1a gender

### Sig Results ----
## eur: p = 0.0477, beta = -0.13912 (compared to avg)
## cisf: p < 0.0001, beta = -0.45649 (compared to avg)
## gnc: p = 0.003567, beta = 1.15136 (compared to avg)
...
# 1a sex: aic = 12508.3, bic = 12562.9
# all: choose model 1a gender

### Sig Results ----
## income: p = 0.000384, beta = -0.106393 (compared to avg)
## cisf: p < 0.0001, beta = -0.478424 (compared to avg)
## gnc: p < 0.0001, beta = 0.993643 (compared to avg)
...
# 1a gender: aic = 8393.2, bic = 8453.9
# 1a sex: aic = 8410.4, bic = 8465.1
# all: choose model 1a gender

### Sig Results ----
## income: p < 0.0001, beta = -0.064502 (compared to avg)
## cisf: p < 0.0001, beta = -0.164060 (compared to avg)
## gnc: p < 0.0001, beta = 0.299418 (compared to avg)
...
# 1a gender: aic = 11802.7, bic = 11863.4
# 1a sex: aic = 11810.7, bic = 11865.4
# all: choose model 1a gender

### Sig Results ----
## income: p < 0.0001, beta = -0.12066 (compared to avg)
## cisf: p = 0.00207, beta = -0.20269 (compared to avg)
## gnc: p = 0.00183, beta = 0.36911 (compared to avg)
...
# 1a gender: aic = 14228.4, bic = 14289.1
# 1a sex: aic = 14257.4, bic = 14312.0
# all: choose model 1a gender

### Sig Results ----
## prs: p = 0.0444, beta = 0.03370
## income: p = 0.0189, beta = -0.09270 (compared to avg)
## cisf: p < 0.0001, beta = -0.45326 (compared to avg)
## gnc: p < 0.0001, beta = 1.05477 (compared to avg)
...
# 1a gender: aic = 12797.6, bic = 12858.2
# 1a sex: aic = 12886.1, bic = 12940.7
# all: choose model 1a gender

### Sig Results ----
## prs: p = 0.00815, beta = 0.0355229
## income: p = 0.00732, beta = -0.0848335 (compared to avg)
## cisf: p < 0.0001, beta = -0.6605230 (compared to avg)
## gnc: p < 0.0001, beta = 1.3553761 (compared to avg)
...
# 2a sex: aic = 20820.4, bic = 20911.5
...
# 1a sex: aic = 17996.6, bic = 18051.2
# all: choose model 1a gender

### Sig Results ----
## cisf: p = 0.029826, beta = -0.37621 (compared to avg)
## gnc: p = 0.000135, beta = 1.19104 (compared to avg)
...
# 1a sex: aic = 20666.1, bic = 20720.7
# all: choose model 1b gender

### Sig Results ----
## cisf: p = 0.0139, beta = -0.650997 (compared to avg)
## gnc: p < 0.0001, beta = 2.319746 (compared to avg)
## prs*gnc: p = 0.0368, beta = 0.401572 (compared to avg)
...
# 1a sex: aic = 18859.6, bic = 18914.2
# all: choose model 1a gender

### Sig Results ----
## income: p = 0.0187, beta = -0.19207
## gnc: p = 0.0196, beta = 0.83332 (compared to avg)
